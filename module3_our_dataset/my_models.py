...
class Network(nn.Module):
    ...
    def __init__(self, base_detector, conf_thresh):
        ...
        self.lidar_cnn_layers = cnn_layers_3() # Similar architecture to radar_cnn_layers
    ...
    def forward(self, images, maps, lidar_maps, radar_boxes_location, lidar_boxes_location, model_mode=0, targets=None):
        """
        Arguments:
        ...
        lidar_score_map = self.lidar_cnn_layers(lidar_maps) # Process lidar maps
        ...
        if len(lidar_boxes_location)>0:
            lidar_boxes_location[:, 1:] *= images.shape[-1]
        box_locations = torch.cat((box_locations, lidar_boxes_location), 0) # Append lidar boxes

        # RoI cropping
        cropped_image_feature = ps_roi_align(roi_score_map, box_locations, (7,7), spatial_scale=1./16)
        cropped_lidar_feature = roi_align(lidar_score_map, box_locations, (7,7), spatial_scale=1./16) # Crop lidar features
        ...
        #aggregate raw lidar proposals
        lidar_boxes = torch.cat((
            lidar_boxes_location,   # n*5, for idx in a batch and location
            refinement_vector[num_img_boxes + len(radar_boxes_location):],  # box confidence, the highest class score
            torch.zeros((len(lidar_boxes_location), 1)).to(self.device),    # class predicted
            refinement_vector[num_img_boxes + len(radar_boxes_location):, 1:], # class scores for c categories
            ), -1)

        boxes = torch.cat((img_boxes, radar_boxes, lidar_boxes), 0) # Append lidar boxes
